...
for file in os.listdir(dir_messy):
    ...
    image = cv2.resize(image, (128, 128))  # すべての画像を同じサイズにリサイズ
    images.append(image)
...
image = cv2.imread(os.path.join(dir_messy, file))
if image is None:
    print(f"Failed to load image at {os.path.join(dir_messy, file)}")
else:
    image = cv2.resize(image, (128, 128))  # すべての画像を同じサイズにリサイズ
    images.append(image)
    labels.append(0)  # 散らかった部屋を0とラベル付け
...
st.title("CleanSnap: あなたの部屋はきれいですか")
...
if uploaded_file is not None:
    ...
    if prediction < 0.5:
        st.write("汚い部屋ですね、入りたくないです！")
    else:
        st.write("あなたの部屋はとても整頓されていて、美しく感じますね！さすがです！")
